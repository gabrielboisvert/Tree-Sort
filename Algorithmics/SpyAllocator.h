...
public:
	...
	void    NotifyDealloc(void* address, size_t count)
	{
		...
		if (count == 0)
		{
			std::printf("*** Deallocate: memory area was empty.\n");
		}
		else
		{
...
extern bool g_mockFailedAllocations; // Used to generate fake bad allocations.
...
struct SpyAllocator : std::allocator<T>
{
	...
	{
		if (g_mockFailedAllocations) // Used to generate fake bad allocations and test exception-safety of calling code.
		{
			return nullptr;
		}
